...
class HeadshotsurtPlugin(Plugin):
    ...
    def __init__(self, console, config=None):
        ...
        try:
            ...
            self.warning(
                "could not get UT_MOD_HEGRENADE value from B3 parser. %s" % err
            )
        ...
        try:
            ...
            if self.console.gameName == "iourt41":
                ...
                self.console.gameName == "iourt42" or self.console.gameName == "iourt43"
            ):
                self.HL_HEAD = "1"
    ...
    def onLoadConfig(self):
        try:
            self._reset_headshots_stats = self.config.getboolean(
                "settings", "reset_headshots"
            )
        except Exception as err:
            self.warning(
                "Using default value %s for reset_headshots. %s"
                % (self._reset_headshots_stats, err)
            )
        ...
        try:
            ...
            self.warning(
                "Using default value %s for show_awards. %s" % (self._show_awards, err)
            )
        self.debug("show awards : %s" % self._show_awards)

        try:
            self._min_level_headshots_cmd = self.config.getint(
                "settings", "min_level_headshots_cmd"
            )
        except Exception as err:
            self.warning(
                "Using default value %s for min_level_headshots_cmd. %s"
                % (self._min_level_headshots_cmd, err)
            )
    ...
    def handle_kills(self, event):
        ...
        if client:
            ...
            if weapon not in (
                self.UT_MOD_BLED,
                self.UT_MOD_HEGRENADE,
            ) and self.is_headshot(hitlocation):
                stats.headshots += 1
    ...
    def show_message(self, client):
        ...
        self.console.write(
            "%s^3 made ^6%s^3 headshot%s"
            % (
                self.coloredClientName(client),
                s.headshots,
                "s" if s.headshots > 1 else "",
            )
        )
        ...
        if data is None or data == "":
            if client is not None:
                ...
                if s.headshots > 0:
                    cmd.sayLoudOrPM(
                        client,
                        "^7You made ^2%s^7 headshot%s"
                        % (s.headshots, "s" if s.headshots > 1 else ""),
                    )
                else:
            ...
            if headshotsStats.headshots > 0:
                client.message(
                    "^7%s made ^2%s^7 headshots"
                    % (sclient.name, headshotsStats.headshots)
                )
            else:
        ...
        if self._show_awards:
            ...
            for c in self.console.clients.getList():
                ...
                if stats.kills > 0:
                    ...
                    self.debug(
                        "[%s] HS:%s K:%s => ratio %.2f"
                        % (c.name, stats.headshots, stats.kills, ratio)
                    )

            for c, ratio in maxHsClients:
                # self.debug("Most HS found : %s (%s)" % (c.name, maxHs))
                self.console.say(
                    "^2Most HS Award : %s^3 (^6%s^3 HS, ratio: %.2f)"
                    % (self.coloredClientName(c), maxHs, ratio)
                )
